...
const SearchBar = ({
  ...
  return (
    <div
      className={`!h-[40px] !min-h-[40px] w-full mx-auto border-[0.5px] border-solid rounded  justify-start items-center relative transition-all duration-150 ease-in-out !z-0
      ${
        active
    ...
    >
      ...
      {value?.length && onSearch ? (
        <span
          className="text-sm text-white cursor-pointer rounded-md px-2 py-1 hover:bg-blue-bright mr-3 bg-black transition-colors ease-in-out duration-300"
          onClick={(e) => {
            setActive(false);
            e.preventDefault();
            e.stopPropagation();
