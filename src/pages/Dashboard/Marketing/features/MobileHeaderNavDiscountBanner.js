...
const MobileHeaderNavDiscountBanner = () => {
  const { warehouse_id } = useParams();

  const {
    loadingMobileHeaderNavDiscountBanners,
    getMobileHeaderNavDiscountBanners,
    mobileHeaderNavDiscountBanners,
  } = MarketingStore;
  const { headerNavs, loading, getHeaderNavs } = CategoriesStore;
  useEffect(() => {
    getMobileHeaderNavDiscountBanners({ data: { page: 1 } });
    getHeaderNavs();
  }, []);

  const forYouMobileHeaderNavDiscountBanner =
    mobileHeaderNavDiscountBanners?.filter((_) => _?.isForYou);
  const categoryMobileHeaderNavDiscountBanner =
    mobileHeaderNavDiscountBanners?.filter((_) => !_?.isForYou);

  const groupedMobileHeaderNavDiscountBanners = groupBy(
    categoryMobileHeaderNavDiscountBanner,
    "headerNavId"
  );
            ...
            <div className="flex flex-col justify-start items-start gap-4 relative w-full">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 smlg:grid-cols-4 gap-4 justify-between items-start w-full mb-5">
                {postArray?.map((item, i) => {
                  const slide = forYouMobileHeaderNavDiscountBanner?.[i];
                  return (
                    <Link
                      key={i + "card"}
                      to={
                        slide
                          ? `/dashboard/marketing/edit-mobile-discount-banner/${warehouse_id}/forYou/${slide?.id}`
                          : `/dashboard/marketing/add-mobile-discount-banner/${warehouse_id}/forYou`
                      }
              ...
              {headerNavs?.map((category) => {
                return (
                  ...
                  >
                    <span className="text-black text-lg sm:text-xl font-700 my-5">
                      Discount Banners ({category?.name})
                    </span>
                    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 smlg:grid-cols-4 gap-4 justify-between items-start w-full mb-5">
                      {postArray?.map((item, i) => {
                        const slides =
                          groupedMobileHeaderNavDiscountBanners?.[category?.id];
                        ...
                        return (
                          <Link
                            ...
                            to={
                              slide
                                ? `/dashboard/marketing/edit-mobile-discount-banner/${warehouse_id}/${category?.id}/${slide?.id}`
                                : `/dashboard/marketing/add-mobile-discount-banner/${warehouse_id}/${category?.id}`
                            }
                            className="flex justify-center items-center cursor-pointer min-w-[187px]  max-w-[187px] min-h-[275px]  max-h-[275px] bg-[#F8F8F8] rounded-[7px] border-[0.8px] border-grey-border hover:border-blue transition-colors duration-500 ease-in-out gap-2.5 snap-center"
                          >
                            {slide ? (
...
export default observer(MobileHeaderNavDiscountBanner);
