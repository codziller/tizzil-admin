...
const Form = observer(() => {
  ...
  const {
    ...
    editMobileHeaderNavDiscountBanner,
    mobileHeaderNavDiscountBanner,
    loadingMobileHeaderNavDiscountBanner,
  } = MarketingStore;

  const defaultValues = {
    headerNavId: position !== "forYou" ? position : "",
    imageUrl: media_id ? mobileHeaderNavDiscountBanner?.imageUrl : [],
    dataId: media_id ? mobileHeaderNavDiscountBanner?.dataId : "",
    pageToLinkTo: media_id
      ? mobileHeaderNavDiscountBanner?.pageToLinkTo
  ...
  const handleOnSubmit = async () => {
    ...
    try {
      ...
      if (!media_id) {
        await createMobileHeaderNavDiscountBanner({
          data: payload,
          onSuccess: () => navigate(-1),
        });
        return;
      } else {
        await editMobileHeaderNavDiscountBanner({
  ...
  return loadingMobileHeaderNavDiscountBanner ? (
    ...
    <>
              ...
              <div className="mb-5 w-full flex justify-between gap-3">
                ...
                {media_id ? (
                  <Button
                    ...
                    text="Delete Mobile Discount Banner"
                    redBg
                  />
                ) : null}
              </div>
              ...
              >
                ...
                <div className="flex flex-col basis-1/3 justify-start items-start gap-y-3 overflow-y-auto">
                  ...
                  <span className="text-grey-text text-sm mb-3 -mt-2">
                    Select the page type to be linked with this discount banner
                  </span>
      ...
      <DetailsModal
        ...
        details={{
          pageType: "Mobile Discount Banner",
          modalType: "delete",
          id: media_id,
        }}
        toggler={() => handleChangeTwo("modalType", false)}
...
const AddHomePagePost = () => {
  ...
  const {
    loadingMobileHeaderNavDiscountBanner,
    getMobileHeaderNavDiscountBanner,
  } = MarketingStore;

  useEffect(() => {
    media_id && getMobileHeaderNavDiscountBanner({ data: { id: media_id } });
  }, []);
  return loadingMobileHeaderNavDiscountBanner ? (
    <div className="w-full flex justify-center items-center min-h-[150px]">
      <CircleLoader blue />
